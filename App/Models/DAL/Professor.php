...
class Professor extends Cadastrousuario{
     ...
     * NÃO TLOGINZ O ATRIBUTO LOGIN, POR SER ESPECIFICO DE ALUNO
     */
     ...
     */
    public function salvar($NOME, $LOGIN, $COD_CURSO, $COD_CAMPUS, $EMAIL, $COD_PERFIL, $SENHA){
        try {
            $dbh = $this->getDb();
            $stmt = $dbh->prepare(
                    '
                        INSERT INTO una_projetos_db.tbl_usuario
                        (nome, login, cod_curso, cod_campus, email, cod_perfil, senha)
                        VALUES
                        (:nome, :login, :cod_curso, :cod_campus, :email, :cod_perfil, :senha);
                    ');
            ...
            if ($e->getCode()==23505){
                $this->modal("Aluno já cadastrado logindo na base de dados!");
            }
            else{
                $this->modal("Favor entrar em contato com o administrador do sistema e informar o código: " . $e->getCode());
            }
        } 
    }
